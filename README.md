...
- 登录会话有效期为 24 小时，刷新页面或打开同站点的新标签页可自动恢复；退出登录、过期或禁用账户后失效。升级前的临时登录状态需重新登录一次。
...
- 模型生成完整题目 JSON、至少 8 组不同输入的测试数据、Python 或 C++14 参考解答、测试覆盖说明和算法解释。
- 用受限执行器运行参考解答，再另行生成输入校验器和第二份解答，对全部样例和测试点交叉核对。
- 命题修订不设轮数及任务总时长上限，直到校验成功、用户中断或模型服务错误。修订支持字段补丁，输入合法且两份解答一致时可自动修正标准输出。未通过的题目不会写入题库。具体过程见 [AI 命题校验流程](docs/AI命题校验流程.md)。
...
输入/输出 Token、计价单位、单价、币种和当前任务费用均会显示。生成、独立审核及修订调用累计计费。提供商返回完整用量时采用原始用量；流式过程中、缺少用量或中断时使用字符数除以 4 向上取整，并明确标注估算。中文及隐藏推理 Token 的偏差可能较大，最终账单以提供商为准。价格由用户按实际报价配置，默认 0 不表示模型免费。

本项目没有内置或代填付费模型密钥。自动测试使用可控的模拟流式服务验证调用协议、中断、重试和费用；真实命题需用户配置自己的提供商后使用。

浏览器登录状态通过短时、一次性凭证交换为后端签发的 HttpOnly Cookie，密码及会话令牌不会写入 URL 或 localStorage。默认允许前端来源 `http://127.0.0.1:8501` 和 `http://localhost:8501`；修改前端地址时设置后端 `OJ_FRONTEND_ORIGINS`（逗号分隔）。浏览器与服务端访问后端的地址不同时，在前端设置 `OJ_BROWSER_API_URL`。前后端应使用相同主机名，以便前端刷新时读取浏览器发来的 Cookie；HTTPS 部署应设置 `OJ_COOKIE_SECURE=1`。
